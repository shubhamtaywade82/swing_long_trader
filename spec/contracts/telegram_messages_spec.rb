...
RSpec.describe "Telegram Messages Contract", type: :contract do
  ...
  describe "message formatting" do
    ...
    it "does not exceed Telegram character limit" do
      # Create a large candidate list
      candidates = Array.new(100) do |i|
        {
          symbol: "STOCK#{i}",
          score: 80.0 + i,
          ai_score: 75.0 + i,
          direction: "long",
          metadata: { trend_alignment: %w[EMA RSI MACD] * 5 },
        }
      end
    ...
    it "all message types should return strings" do
      ...
      expect(Telegram::AlertFormatter.format_portfolio_snapshot({ total_value: 100_000.0, total_pnl: 0.0, total_pnl_pct: 0.0, positions: [] })).to be_a(String)
    end
  end
end
