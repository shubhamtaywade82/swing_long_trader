...
RSpec.describe Backtesting::Position, type: :service do
  ...
  describe '#close' do
    it 'closes position with exit details' do
      ...
      expect(position.exit_date).to eq(Date.today + 1)
  ...
  describe '#closed?' do
    ...
    context 'when position is closed' do
      before do
        position.close(exit_date: Date.today, exit_price: 110.0, exit_reason: 'tp_hit')
      end
  ...
  describe '#check_exit' do
    context 'for long position' do
      it 'returns exit info when stop loss is hit' do
        result = position.check_exit(94.0, Date.today)
      ...
      it 'returns exit info when take profit is hit' do
        result = position.check_exit(110.0, Date.today)
      ...
      it 'returns nil when no exit condition is met' do
        result = position.check_exit(102.0, Date.today)
    ...
    context 'for short position' do
      ...
      it 'returns exit info when stop loss is hit' do
        result = short_position.check_exit(106.0, Date.today)
      ...
      it 'returns exit info when take profit is hit' do
        result = short_position.check_exit(90.0, Date.today)
      ...
      it 'returns nil when no exit condition is met' do
        result = short_position.check_exit(98.0, Date.today)
    ...
    context 'when position is already closed' do
      ...
      it 'returns nil' do
        result = position.check_exit(120.0, Date.today)
