...
RSpec.describe Screeners::LongtermScreener, type: :service do
  ...
  describe "#call" do
    context "when instruments are provided" do
      ...
      before do
        allow(instrument).to receive_messages(has_candles?: true, load_daily_candles: create(:candle_series, candles: create_list(:candle, 150)), load_weekly_candles: create(:candle_series, candles: create_list(:candle, 30)))
    ...
    context "when no instruments provided" do
      ...
      it "loads from universe file" do
        ...
        allow(Instrument).to receive(:where).with(instrument_type: %w[EQUITY INDEX]).and_return(Instrument.none)
    ...
    context "when instrument has insufficient data" do
      ...
      before do
        allow(instrument).to receive_messages(has_candles?: true, load_daily_candles: create(:candle_series, candles: create_list(:candle, 50)))
      end
    ...
    context "when limit is specified" do
      ...
      before do
        instruments.each do |inst|
          allow(inst).to receive_messages(has_candles?: true, load_daily_candles: create(:candle_series, candles: create_list(:candle, 150)), load_weekly_candles: create(:candle_series, candles: create_list(:candle, 30)))
        end
        allow_any_instance_of(described_class).to receive(:calculate_indicators).and_return(
          { rsi: 65, ema20: 100.0 },
        )
    ...
    context "with edge cases" do
      ...
      it "handles insufficient weekly candles" do
        allow(instrument).to receive_messages(has_candles?: true, load_daily_candles: create(:candle_series, candles: create_list(:candle, 150)), load_weekly_candles: create(:candle_series, candles: create_list(:candle, 10)))
