  ...
  module Swing
    ...
    class ExitMonitorJob < ApplicationJob
      ...
      def check_exit_conditions(order)
        ...
        # Check stop loss
        if stop_loss
          if order.buy? && current_price <= stop_loss
            return { should_exit: true, reason: 'Stop loss triggered' }
          elsif order.sell? && current_price >= stop_loss
            return { should_exit: true, reason: 'Stop loss triggered' }
          end
        end
        ...
        # Check take profit
        if take_profit
          if order.buy? && current_price >= take_profit
            return { should_exit: true, reason: 'Take profit triggered' }
          elsif order.sell? && current_price <= take_profit
            return { should_exit: true, reason: 'Take profit triggered' }
          end
        end
        ...
        holding_days = (Date.today - entry_date).to_i
      ...
      def place_exit_order(order, reason)
