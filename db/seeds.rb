...
if last_import_raw.blank?
  puts "Skipping seed: no instrument import recorded. Run `bin/rails instruments:import` first."
else
  imported_at = Time.zone.parse(last_import_raw.to_s) rescue nil
  if imported_at.nil?
    puts "Skipping seed: could not parse last import timestamp (#{last_import_raw.inspect})."
  else
    max_age = InstrumentsImporter::CACHE_MAX_AGE
    age = Time.current - imported_at
    if age > max_age
      puts "Skipping seed: import is stale (age=#{age.round(1)}s > #{max_age.inspect}). Run `bin/rails instruments:reimport`."
    else
      ...
      queries.each do |q|
        ...
        if instrument
          puts "✅ Found #{q[:label]}: security_id=#{instrument.security_id}, symbol=#{instrument.symbol_name}"
          found += 1
        else
          puts "⚠️  #{q[:label]} not found (exchange=#{q[:exchange]} segment=index symbol_name ILIKE #{q[:symbol_like]})"
        end
      end
