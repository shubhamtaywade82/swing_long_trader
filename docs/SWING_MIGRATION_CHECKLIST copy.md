...
- [ ] Copy `app/models/instrument_type_mapping.rb` (OPTIONAL - only if needed)
- [ ] Copy `app/models/watchlist_item.rb` (MODIFY - remove derivative enum)
- [ ] Copy `app/models/derivative.rb` (SKIP - not needed for stocks-only)
...
### Data Import & Setup
- [ ] **REWRITE** `app/services/instruments_importer.rb` (See SWING_CLEAN_IMPORTER_GUIDE.md)
- [ ] **MODIFY** `lib/tasks/instruments.rake` (Remove derivative stats)
- [ ] Create `lib/tasks/universe.rake` (NEW - universe builder)
- [ ] Copy `db/seeds.rb` (modify heavily - remove scalper watchlist)
- [ ] Download NSE index CSVs to `config/universe/csv/`
- [ ] Build master universe: `rails universe:build`
...
- [ ] Create `create_watchlist_items` migration
  - [ ] Add `segment`, `security_id`, `kind`, `label`, `active`
  - [ ] Add polymorphic `watchable_type`, `watchable_id`
  - [ ] Add unique index on `[segment, security_id]`
  - [ ] Add index on `[watchable_type, watchable_id]`
- [ ] Run `rails db:migrate`
...
- [ ] Download NSE index CSVs to `config/universe/csv/`
  ...
  - [ ] Sector indices (Bank, IT, FMCG, Pharma, Auto, etc.)
- [ ] Build master universe: `rails universe:build`
- [ ] Verify universe: `rails universe:stats`
- [ ] **REWRITE** `app/services/instruments_importer.rb` (stocks-only, see guide)
- [ ] **MODIFY** `lib/tasks/instruments.rake` (remove derivative stats)
- [ ] Create `lib/tasks/universe.rake` (universe builder)
...
- [ ] Verify no derivatives: `rails runner "puts Instrument.where(segment: 'D').count"` (should be 0)
- [ ] Verify only NSE_EQ: `rails runner "puts Instrument.where(exchange: 'NSE', segment: 'EQ').count"`
